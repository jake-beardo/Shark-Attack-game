...
class Fish(object):
    ...
    def update(self):
        ...
        if self.vel < 0 and self.type == Fish4:
            self.x += self.vel
            self.y += self.vely
            if self.x < 0:
                self.x = self.startx
...
def start_screen():
    ...
    while Start:
        ...
        LargePieceOfText = "you have 1 minute to eat as many high scoring fish as possible\n" \
                           "use the left, right and up keys on your keyboard to catch the fish in the shark's mouth" \
        ...
        Screen.blit(Fish1Image, (DisplayWidth - 80, 8))
...
def head_first():
    ...
    while Running:
        ...
        if shark.hitbox[0] < fish5.hitbox[0] + fish5.hitbox[2] and shark.hitbox[0] + shark.hitbox[2] > fish5.hitbox[0]:
            if shark.hitbox[1] + shark.hitbox[3] > fish5.hitbox[1] + fish5.hitbox[3] \
                    and shark.hitbox[1] < fish5.hitbox[1]:
                print("hit")
                NomSound.play()
                fish5.x = 900
                score += 7
